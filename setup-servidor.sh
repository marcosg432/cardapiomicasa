...
if [ -f "$BEVERAGE_FILE" ]; then
    # Fazer backup
    cp "$BEVERAGE_FILE" "${BEVERAGE_FILE}.backup.$(date +%Y%m%d_%H%M%S)" 2>/dev/null || true
    
    echo "  🔧 Aplicando correção no arquivo beverages/[id].tsx..."
    
    # Aplicar todas as correções possíveis (múltiplas variações)
    # Correção 1: formato com espaço
    ...
    # Correção 2: formato sem espaço após ?
    sed -i "s/price: typeof formData\.price === 'string'? Number(formData\.price\.replace(',', '.')) : (formData\.price || 0),/price: typeof formData.price === 'string' ? Number(formData.price.replace(',', '.')) : (typeof formData.price === 'number' ? formData.price : 0),/g" "$BEVERAGE_FILE"
    
    # Correção 3: formato com espaço em formData. price
    sed -i "s/formData\. price/formData.price/g" "$BEVERAGE_FILE"
    
    # Correção 4: display_order
...
# Garantir que deploy.sh existe (criar novamente se necessário)
if [ ! -f "deploy.sh" ]; then
    echo ""
    echo "⚠️  deploy.sh ainda não existe, criando agora..."
    cat > deploy.sh << 'DEPLOYEOF'
#!/bin/bash
